...
/*   Updated: 2025/01/08 11:28:31 by jingwu           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_camera	create_camera_ray(t_camera *camera, t_vec2 *pixel)
{
	...
	normal_y = (1.0f - (2.0f * (pixel->y + 0.5f) / WIN_HEIGHT)) * tan(camera->fov * DEGREE_TO_RADIAN);
	...
	right_vector = vec3_scale(camera->right, normal_x);
	up_vector = vec3_scale(camera->up, normal_y);
	added = vec3_add(right_vector, up_vector);
	forward_vector = vec3_add(camera->direction, added);
	vec3_normalise(&forward_vector);// for some reason can't use not point parameter here.
